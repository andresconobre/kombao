                ...
                <div v-for="({ nome, lowername, fotos, tags }, index) in membros" :key="index">
                    <!-- <RouterLink :to="`/membros/${lowername}`"> -->
                        <swiper :slides-per-view="1" class="rounded-md group border-2 dark:border-gray-light border-gray-dark bg-gray-light relative" navigation :modules="modules">
                            <swiper :breakpoints="{ 0:{ slidesPerView:1.5 }, 768:{ slidesPerView:2 } }" :space-between="20" loop autoplay :modules="[Autoplay]" class="!absolute w-full top-0 !px-4 mt-2 lg:!opacity-0 lg:group-hover:!opacity-100 transition duration-300 hover:cursor-grab">
                            ...
                            <swiper-slide v-for="foto in fotos" :key="foto">
                                <img :src="`/img/${lowername}/${lowername}${foto}.png`" :alt="nome" class="object-cover aspect-[3/4] h-full w-full transition duration-300 ease-in-out -translate-y-10 lg:-translate-y-0 lg:group-hover:-translate-y-10 z-10 relative hover:cursor-grab">
                            </swiper-slide>
